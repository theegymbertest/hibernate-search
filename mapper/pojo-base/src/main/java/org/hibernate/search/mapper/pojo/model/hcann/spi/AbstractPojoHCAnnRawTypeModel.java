...
public abstract class AbstractPojoHCAnnRawTypeModel<T, I extends AbstractPojoHCAnnBootstrapIntrospector>
	...
	public Optional<PojoTypeModel<?>> typeArgument(Class<?> rawSuperType, int typeParameterIndex) {
		return rawTypeDeclaringContext.genericTypeContext().resolveTypeArgument( rawSuperType, typeParameterIndex )
				.map( rawTypeDeclaringContext::createTypeArgumentModel );
	}

	@Override
	public Optional<PojoTypeModel<?>> arrayElementType() {
		return rawTypeDeclaringContext.genericTypeContext().resolveArrayElementType()
				.map( rawTypeDeclaringContext::createTypeArgumentModel );
	}
