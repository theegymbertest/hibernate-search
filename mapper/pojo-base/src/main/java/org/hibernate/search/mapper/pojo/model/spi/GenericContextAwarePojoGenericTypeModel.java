...
public final class GenericContextAwarePojoGenericTypeModel<T>
	...
	private static GenericContextAwarePojoGenericTypeModel<?> createTypeArgumentModel(Helper helper,
			GenericTypeContext selfTypeContext, Type declaredType) {
		return new GenericContextAwarePojoGenericTypeModel<>( helper,
				new GenericTypeContext( selfTypeContext.declaringContext(), declaredType ) );
	}
	...
	public static class RawTypeDeclaringContext<T> {
		...
		public GenericTypeContext genericTypeContext() {
			return genericTypeContext;
		}

		public PojoTypeModel<?> createTypeArgumentModel(Type declaredType) {
			return GenericContextAwarePojoGenericTypeModel.createTypeArgumentModel( helper, genericTypeContext, declaredType );
		}
		 ...
		 * and so on.
		 */
		public PojoTypeModel<?> createPropertyTypeModel(Type declaredType) {
			/*
	...
	public Optional<PojoTypeModel<?>> typeArgument(Class<?> rawSuperType, int typeParameterIndex) {
		return genericTypeContext.resolveTypeArgument( rawSuperType, typeParameterIndex )
				.map( type -> createTypeArgumentModel( helper, genericTypeContext, type ) );
	}
