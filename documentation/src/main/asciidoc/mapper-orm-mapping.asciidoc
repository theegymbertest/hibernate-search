...
Hibernate Search uses the `java.time` APIs introduced in Java 8 to model date/time values internally.
As a result, legacy date/time objects such as `java.util.Calendar` or `java.util.Date`
need to be converted to a `java.time` type before they can be indexed.
...
and will assume it represents the same date/time in Java 8 APIs.
