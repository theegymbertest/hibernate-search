...
Where possible, the version running on your cluster will be automatically detected on startup,
and Hibernate Search will adapt based on the detected version.
When detection is not possible (e.g. with link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless]),
or if your application must not make an HTTP call to the cluster on startup,
you can configure the version explicitly:
...
[cols="h,5*",options="header"]
|===============
| 2+|Elasticsearch 2+|OpenSearch |link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless]
||7.x|8.x|1.x|2.x|
|<<backend-elasticsearch-configuration-version-check,Distribution/version detection on startup>> 4+|Available |Not available, must be configured explicitly
|Explicit <<indexing-workspace-flush,flush>>, <<indexing-workspace-refresh,refresh>>, and <<indexing-workspace-merge-segments,merge segments>> 4+|Available |Not available
|<<backend-elasticsearch-index-lifecycle,Minimal index status requirement>> for schema management 4+|Available |Not available
|===============

[[backend-elasticsearch-compatibility-amazon]]
=== Amazon OpenSearch Service

Amazon's managed OpenSearch services (both link:{amazonOpenSearchServiceUrl}[classic]
and link:{amazonOpenSearchServerlessUrl}[serverless])
require <<backend-elasticsearch-configuration-aws,proprietary authentication>> that involves extra configuration.

Also, some operations are not available on link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless],
in particular:
...
or link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless],
...
[NOTE]
====
The check is already disabled by default for link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless],
because it doesn't allow retrieving the underlying version of OpenSearch.
====
...
except on link:{amazonOpenSearchServerlessUrl}[Amazon OpenSearch Serverless]
where index status checks are skipped because Amazon OpenSearch Serverless does not support index status checks.
