...
The `knn` predicate matches k nearest documents to a query vector based on the distance to the given vector field values.
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/LucenePredicateDslIT.java[tags=knn]
----
====
...
For example, if an entity property is of a byte array type (`byte[]`),
...
[source, JAVA, indent=0, subs="+callouts"]
----
include::{sourcedir}/org/hibernate/search/documentation/search/predicate/LucenePredicateDslIT.java[tags=knn-filter]
----
====
