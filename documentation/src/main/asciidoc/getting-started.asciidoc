...
To that end, you can use the `SearchIndexer` API, as shown in the following code:

.Using the `SearchIndexer` API to manually (re)index the already persisted data
====
[source, JAVA, indent=0]
----
...
<2> Create an "indexer", passing the entity types you want to index. Pass `Object.class` to index all of them.
<3> Call `reindex` to start reindexing.
<4> You may pass a lambda expression to fine-tuned the indexing process.
For example you can set the number of threads to be used. For the complete option list see <<manual-index-changes>>.
<5> By default, indexer methods are asynchronous. Call `.join()` on the returned object to wait for the operation to finish.
====
