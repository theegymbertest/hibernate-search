	...
	private JsonObject convertToJson(Document document, Class<?> entityType) {
		...
		for ( IndexableField field : document.getFields() ) {
			if ( field instanceof NestingMarker ) {
				...
				treeBuilder.reset();
				treeBuilder.append( ((NestingMarker) field).getPath() );
			...
			if ( embeddedType != null && fieldPath.equals( embeddedType.getEmbeddedNullFieldName() ) ) {
				...
				switch ( containerType ) {
					...
					case MAP:
						...
						treeBuilder.addPropertyAbsolute( fieldPath, value != null ? new JsonPrimitive( value ) : null );
						break;
					case OBJECT:
					...
					if ( SpatialHelper.isSpatialField( fieldPath ) ) {
						if ( isNumeric( field ) && ( SpatialHelper.isSpatialFieldLatitude( fieldPath ) ||
							...
							if ( SpatialHelper.isSpatialFieldLatitude( fieldPath ) ) {
								treeBuilder.addPropertyAbsolute( spatialPropertyPath + ".lat", value != null ? new JsonPrimitive( value ) : null );
							}
							else if ( SpatialHelper.isSpatialFieldLongitude( fieldPath ) ) {
								treeBuilder.addPropertyAbsolute( spatialPropertyPath + ".lon", value != null ? new JsonPrimitive( value ) : null );
							}
						}
						else {
						...
						if ( stringValue != null ) {
							treeBuilder.addPropertyAbsolute( fieldPath, new JsonPrimitive( stringValue ) );
						}
						else if ( numericValue != null ) {
							treeBuilder.addPropertyAbsolute( fieldPath, new JsonPrimitive( numericValue ) );
						}
						else {
							treeBuilder.addPropertyAbsolute( fieldPath, null );
						}
					}
				}
				else {
				...
				treeBuilder.addPropertyAbsolute( facetFieldPath, value != null ? new JsonPrimitive( value ) : null );
			}
