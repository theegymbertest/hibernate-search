...
class LuceneVectorFieldIT {
	...
	private static class NestedIndexBinding {
		...
		NestedIndexBinding(IndexSchemaElement root) {
			...
			byteVector = nestedField.field( "byteVector", f -> f.asByteVector().dimension( 2 ).projectable( Projectable.YES ) )
					.toReference();
			floatVector = nestedField
					.field( "floatVector", f -> f.asFloatVector().dimension( 2 ).projectable( Projectable.YES ) ).toReference();
		}
	}
