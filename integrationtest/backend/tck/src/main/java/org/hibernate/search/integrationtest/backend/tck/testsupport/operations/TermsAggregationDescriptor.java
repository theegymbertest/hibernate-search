...
public class TermsAggregationDescriptor extends AggregationDescriptor {
	...
	public <F> ExpectationsAlternative<
		...
		Map<F, Integer> mainIndexExpected = new LinkedHashMap<>();
		Map<F, Integer> mainAndOtherIndexExpected = new LinkedHashMap<>();
		Map<F, Integer> multiValuedIndexExpected = new LinkedHashMap<>();

		// Simple dataset for the main index: strictly decreasing number of documents for each term
		int numberOfDocuments = uniqueTermValues.size();
		...
		multiValuedIndexExpected.put( typeDescriptor.toExpectedDocValue( uniqueTermValues.get( 0 ) ), 3 );
		multiValuedIndexExpected.put( typeDescriptor.toExpectedDocValue( uniqueTermValues.get( 1 ) ), 2 );
		...
		return ExpectationsAlternative.supported( new SupportedSingleFieldAggregationExpectations<F>(
			...
			@Override
			public <T> AggregationScenario<Map<T, Integer>> withFieldType(TypeAssertionHelper<F, T> helper) {
				return doCreate( mainIndexExpected, helper );
			}

			@Override
			public <T> AggregationScenario<Map<T, Integer>> withFieldTypeOnMainAndOtherIndex(TypeAssertionHelper<F, T> helper) {
			...
			private <T> AggregationScenario<Map<T, Integer>> doCreate(Map<F, Integer> expectedResult,
					TypeAssertionHelper<F, T> helper) {
				return new AggregationScenario<Map<T, Integer>>() {
					@Override
					public AggregationFinalStep<Map<T, Integer>> setup(SearchAggregationFactory factory, String fieldPath) {
						return factory.terms().field( fieldPath, helper.getJavaClass() );
					}

					@Override
					public AggregationFinalStep<Map<T, Integer>> setupWithConverterSetting(SearchAggregationFactory factory,
							String fieldPath, ValueConvert convert) {
						return factory.terms().field( fieldPath, helper.getJavaClass(), convert );
					}

					@Override
					public void check(Map<T, Integer> aggregationResult) {
