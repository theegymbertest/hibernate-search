...
public class SimpleFieldModelsByType {
	public static <S extends IndexFieldTypeOptionsStep<?, ?>> SimpleFieldModelsByType mapAll(
			Collection<? extends FieldTypeDescriptor<?, S>> typeDescriptors,
	...
	// We can't easily express "S2 extends S and IndexFieldTypeOptionsStep<?, F>" in java
	// so we fall back to dirty, dirty casts...
	@SuppressWarnings("unchecked")
	private static <F, S extends IndexFieldTypeOptionsStep<?, ?>> SimpleFieldModel<?> map(
			FieldTypeDescriptor<F, ? extends S> typeDescriptor, IndexSchemaElement parent, String prefix,
			Consumer<S> additionalConfiguration) {
		return SimpleFieldModel.mapper( (FieldTypeDescriptor<F, ?>) typeDescriptor )
				.map( parent, prefix + typeDescriptor.getUniqueName(),
						step -> additionalConfiguration.accept( (S) step ) );
	}

	@SafeVarargs
	public static <S extends IndexFieldTypeOptionsStep<?, ?>> SimpleFieldModelsByType mapAllMultiValued(
			Collection<? extends FieldTypeDescriptor<?, ? extends S>> typeDescriptors,
			IndexSchemaElement parent, String prefix,
			Consumer<S>... additionalConfiguration) {
	...
	private static <F, S> SimpleFieldModel<?> mapMultiValued(
			FieldTypeDescriptor<F, ? extends S> typeDescriptor, IndexSchemaElement parent, String prefix,
			Consumer<? super S>[] additionalConfiguration) {
		return SimpleFieldModel.mapper( typeDescriptor )
