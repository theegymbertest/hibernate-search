...
public class ElasticsearchRangeAggregation<F, K>
		extends AbstractElasticsearchBucketAggregation<Range<K>, Integer> {
	...
	@Override
	protected Map<Range<K>, Integer> doExtract(AggregationExtractContext context, JsonObject outerObject,
			JsonElement buckets) {
		JsonObject bucketMap = buckets.getAsJsonObject();
		Map<Range<K>, Integer> result = CollectionHelper.newLinkedHashMap( rangesJson.size() );
	...
	public static class Builder<F, K> extends AbstractBuilder<Range<K>, Integer>
		...
		public void range(Range<? extends K> range) {
			...
			if ( range.getLowerBoundValue().isPresent() ) {
				...
				rangeJson.add( "from", convertToFieldValue( range.getLowerBoundValue().get() ) );
			}
			if ( range.getUpperBoundValue().isPresent() ) {
				if ( !RangeBoundInclusion.EXCLUDED.equals( range.getUpperBoundInclusion() ) ) {
					throw log.elasticsearchRangeAggregationRequiresCanonicalFormForRanges( range );
				}
				rangeJson.add( "to", convertToFieldValue( range.getUpperBoundValue().get() ) );
			}
